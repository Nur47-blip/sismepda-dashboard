...
export async function GET() {
  try {
    ...
    const recentDays = await prisma.attendanceDay.findMany({ where: { schoolClass: classWhere }, include: { schoolClass: true, submittedBy: true, attendances: { select: { status: true } } }, orderBy: { submittedAt: "desc" }, take: 7 })
    ...
    const trendDays = await prisma.attendanceDay.findMany({ where: { schoolClass: classWhere }, select: { date: true, attendances: { select: { status: true } } }, orderBy: { date: "desc" } })
